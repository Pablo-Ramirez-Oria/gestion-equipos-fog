...
<x-layouts.app :title="'Gestión de equipos en el FOG - Estados'">

    <h1 class="text-2xl mb-4">Estados</h1>
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
        ...
        <div class="flex items-center justify-between pb-4">
            ...
            <form method="GET" action="{{ route('estados.index') }}">
                <div class="relative">
                    ...
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="inline-flex items-center space-x-2 text-sm">
                                <a href="{{ route('estados.show', $estado) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                                    Ver
                                </a>

                                @role('admin')
                                    <span class="text-gray-400">|</span>
                                    <a href="{{ route('estados.edit', $estado) }}" class="text-yellow-600 hover:underline">
                                        Editar
                                    </a>
                                    <span class="text-gray-400">|</span>
                                    <button type="button" class="text-red-600 hover:underline" data-modal-toggle="modal-eliminar-{{ $estado->id }}">
                                        Eliminar
                                    </button>

                                    <!-- Modal eliminar -->
                                    <div id="modal-eliminar-{{ $estado->id }}" class="modal hidden fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto bg-black bg-opacity-50">
                                        <div class="relative w-full max-w-sm p-6 bg-white dark:bg-gray-800 rounded-lg shadow-lg">
                                            <button type="button" class="absolute top-2 right-2 text-gray-500 dark:text-gray-400" data-modal-hide="modal-eliminar-{{ $estado->id }}">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                                                </svg>
                                            </button>
                                            <h2 class="text-lg font-bold mb-4">
                                                ¿Estás seguro de que deseas eliminar <br> este estado?
                                            </h2>
                                            <p>Esta acción no se puede deshacer.</p>
                                            <div class="mt-4 flex justify-end space-x-2">
                                                <button type="button" class="text-gray-500 dark:text-gray-400 hover:underline" data-modal-hide="modal-eliminar-{{ $estado->id }}">
                                                    Cancelar
                                                </button>
